...
const PricingTable = () => {
    ...
    const badgeRef = useGsapReveal('scaleIn', { delay: 0.1, duration: 0.6 });
    const rowsRef = useRef(null);
    const taxRef = useGsapReveal('clipReveal', { delay: 0.3, duration: 1 });
    ...
    // GSAP stagger pricing rows with slide-in
    useEffect(() => {
        const ctx = gsap.context(() => {
            if (rowsRef.current) {
                const rows = rowsRef.current.querySelectorAll('.pricing-row');
                gsap.fromTo(rows, {
                    opacity: 0,
                    x: -60,
                    scale: 0.97,
